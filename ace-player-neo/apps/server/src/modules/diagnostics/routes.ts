/* Rutas del módulo `diagnostics`.
   ...
   - diagnosticsList: GET /api/v1/diagnostics
   - diagnosticsReport: POST /api/v1/diagnostics

   ESQUELETO: todavía no se registra ningún manejador y app.ts responde 501
   `not_implemented` en todas. Para portar una ruta:
     router.handle('GET', '/api/…', (req, ctx) => services.diagnostics.…);   // antiguas
     router.handle('<id>', (input, ctx) => services.diagnostics.…);          // v1
