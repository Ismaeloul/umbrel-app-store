/* Rutas del módulo `auth`.
   ...
   - pairingCreate: POST /api/v1/pairing
   - pairingClaim: POST /api/v1/pairing/claim
   - devicesList: GET /api/v1/devices
   - deviceRevoke: DELETE /api/v1/devices/:id

   ESQUELETO: todavía no se registra ningún manejador y app.ts responde 501
   `not_implemented` en todas. Para portar una ruta:
     router.handle('GET', '/api/…', (req, ctx) => services.auth.…);   // antiguas
     router.handle('<id>', (input, ctx) => services.auth.…);          // v1
