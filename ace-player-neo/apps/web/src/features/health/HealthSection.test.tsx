...
function mock(extra: Parameters<typeof mockFetch>[0] = {}) {
  net = mockFetch({
    ...
    'GET /api/v1/diagnostics': (call) => {
      ...
      return json(
        cause ? { ...base, entries: base.entries.filter((e) => e.cause === cause) } : base,
      );
    },
...
describe('Salud del sistema', () => {
  it('resumen, cuadrícula por servicio con forma y palabra, y sin sondeos', async () => {
    ...
    expect(
      screen.getByText(
        /^1 fuente en cuarentena · 4 correcciones aprendidas · comprobado \d\d:\d\d$/,
      ),
    ).toBeInTheDocument();
    ...
    await waitFor(() =>
      expect(net.calls.filter((c) => c.url.startsWith('/api/v1/health'))).toHaveLength(1),
    );
  ...
  it('reiniciar el motor pide un segundo toque (6 s) y luego vuelve a mirar', async () => {
    ...
    expect(
      screen.getByText(/Reiniciarlo corta la reproducción en todos los dispositivos/),
    ).toBeInTheDocument();
    ...
    await waitFor(() =>
      expect(net.calls.some((c) => c.method === 'POST' && c.url === '/api/v1/engine/restart')).toBe(
        true,
      ),
    );
    expect((client.getQueryData(routeKey('engineStatus')) as { status: string }).status).toBe(
      'restarting',
    );
    await waitFor(() =>
    ...
    await waitFor(() =>
      expect(net.calls.slice(before).map((c) => c.url)).toEqual(
        expect.arrayContaining(['/api/v1/engine/status']),
      ),
    );
  });
        ...
        json(
          {
            error: {
              ...
              message: 'Espera unos segundos antes de volver a reiniciarlo.',
              requestId: 'r',
            },
          },
          409,
  ...
  it('sin salud: estado de error con salida (regla 32)', async () => {
    ...
    expect(
      await screen.findByRole('heading', { name: 'No se pudo leer la salud' }),
    ).toBeInTheDocument();
    expect(
      screen.getByText('Vuelve a comprobar cuando el NAS esté accesible.'),
    ).toBeInTheDocument();
  ...
  it('avisos del backend tal cual', async () => {
    setup({
      'GET /api/v1/health': {
        ...
        warnings: [
          {
            code: 'auto_restart_exhausted',
            message: 'El motor ya se ha reiniciado 3 veces esta hora.',
          },
        ],
      },
    });
    ...
    expect(
      within(list).getByText('El motor ya se ha reiniciado 3 veces esta hora.'),
    ).toBeInTheDocument();
...
describe('registro de fallos', () => {
  ...
  it('filtrar por causa pide al backend ya filtrado y explica la causa', async () => {
    ...
    expect(
      screen.getByText('El motor AceStream se cayó, no pudo abrir un canal o se reinició.'),
    ).toBeInTheDocument();
    await waitFor(() =>
      expect(net.calls.map((c) => c.url)).toContain(
        `/api/v1/diagnostics?cause=engine&limit=${DIAG_LIMIT}`,
      ),
    );
  ...
  it('por fuente: las que fallan en 24 h, agrupadas', async () => {
    ...
    const list = await screen.findByRole('list', {
      name: 'Fuentes con fallos en las últimas 24 horas',
    });
  ...
  it('sin fallos: lo dice (es buena noticia, no un vacío que resolver)', async () => {
    ...
    expect(
      await screen.findByText('Sin fallos registrados. Todo ha ido bien.'),
    ).toBeInTheDocument();
  });
