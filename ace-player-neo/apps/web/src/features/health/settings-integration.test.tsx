...
beforeEach(() => {
  ...
  net = mockFetch({
    'GET /api/v1/directories': { web: [], webSyncedAt: null, webSources: [], activeWebSourceId: null },
...
describe('Ajustes con Salud y Dispositivos', () => {
  it('las dos secciones existen y salen en el índice de Ajustes', async () => {
    ...
    renderSection(<SettingsView route={{ vista: 'ajustes', seccion: null }} active />, '?vista=ajustes');
    ...
    expect(await screen.findByRole('heading', { name: 'Salud del sistema', level: 2 })).toBeInTheDocument();
    expect(await screen.findByText('Todo funciona.')).toBeInTheDocument();
    expect(await screen.findByRole('button', { name: 'Emparejar un dispositivo' })).toBeInTheDocument();
  });

  it('«ajustes/salud» (el indicador del motor) lleva a Salud', async () => {
    renderSection(<SettingsView route={{ vista: 'ajustes', seccion: 'salud' }} active />, '?vista=ajustes/salud');
    const index = screen.getByRole('navigation', { name: 'Secciones de Ajustes' });
    expect(within(index).getByRole('link', { name: 'Salud' })).toHaveAttribute('aria-current', 'location');
