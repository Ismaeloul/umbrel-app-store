...
| Fichero | Qué |
|---|---|
| `index.tsx` | La vista: cabecera, tira de días, «Para ti»/«Todos», tarjeta de primer uso, lista y, desde 1024 px, escenario + «Luego» (desde 1280, tira de directos B1) |
| `column.tsx` | Columna compacta junto al reproductor (centro de partido ≥ 1280). Comparte día y filtro con la vista (`state.ts`) |
...
| `MatchRow.tsx` | Franja (normal y compacta). `FlipNum`, `CensorBars` y `teamGlow` los usa también el escenario |
| `AgendaList.tsx` | Lista virtualizada (`@tanstack/react-virtual`): con la página (`WindowAgendaList`) o con un contenedor (`ElementAgendaList`) |
| `DayStrip.tsx` | Tira de días (B8 en el móvil, teselas en escritorio) que no se recoloca al repintar |
| `Stage.tsx` | Escenario de escritorio y `LiveStrip` (B1) |
...
- **«Ver canal» / «Buscar canal» abren el centro de partido**, que es quien
  ...
  eso la franja no tiene estado «Buscando» propio (§3.4): lo enseña el partido.
- **Rótulos de canal:** la franja pinta 2 y «+n» (en 360 px no caben 8); el
...
- **Accesibilidad:** una frase `role="status"` resume día, partidos y directos
  (regla 36) en vez de hacer `aria-live` la lista virtualizada.
