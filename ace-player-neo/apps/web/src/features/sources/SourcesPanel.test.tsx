...
function prepare(states: Array<'working' | 'weak' | 'failed' | 'checking' | 'queued'>, extra: Partial<SessionState> = {}) {
  const entries = applyScan(
    startScan(states.map((_, i) => entryFromCandidate(candidate(i + 1))), 3),
    scanJob(states),
  );
  ...
  sessionStore.set({
    ...
    phase: 'ready',
    entries,
    scan: { id: JOB, status: job.status, total: job.total, checked: job.checked, playable: job.playable, retryAt: null },
    ...extra,
  });
}
...
describe('lista (móvil)', () => {
  ...
  it('la que suena lleva «En pantalla» y la barra «Emitiendo» cuenta sobre el total', () => {
    ...
    expect(screen.getByText(/Fuente/, { selector: '.src-now__meta' })).toHaveTextContent('Fuente 1 de 3');
  ...
  it('sin fuente activa dentro de un partido solo queda «Pegar hash» (regla 26)', () => {
    prepare(['failed', 'failed'], { activeHash: null, failureText: 'Ninguna de las 2 fuentes da señal ahora mismo. Prueba "Rebuscar" o pega un Content ID.' });
    renderWithApp(<SourcesPanel variant="list" />);
    const group = screen.getByRole('group', { name: 'Acciones de la fuente' });
    expect(within(group).getAllByRole('button').map((b) => b.textContent)).toEqual(['Pegar hash']);
    expect(screen.getByRole('status')).toHaveTextContent('Ninguna de las 2 fuentes da señal ahora mismo.');
...
describe('inspector y rack (escritorio)', () => {
  it('rack con columnas y todas las acciones de la fuente activa', () => {
    ...
    for (const label of ['Nº', 'Fuente', 'Estado', 'Pares', 'Mbit/s']) expect(screen.getByText(label)).toBeInTheDocument();
    const group = screen.getByRole('group', { name: 'Acciones de la fuente' });
    const names = within(group).getAllByRole('button').map((b) => b.textContent);
    expect(names).toEqual(['Favorito', 'Rebuscar', 'Pegar hash', 'Copiar hash', 'Es el canal correcto', 'Reportar', 'Abrir en…']);
    fireEvent.click(within(group).getByRole('button', { name: 'Abrir en…' }));
    expect(screen.getByRole('menuitem', { name: /Copiar URL del stream \(VLC\)/ })).toBeInTheDocument();
    expect(screen.getByRole('menuitem', { name: /Abrir en la app de AceStream/ })).toBeInTheDocument();
  });
  ...
  it('«Reportar» abre la hoja con los 5 motivos y manda el elegido', async () => {
    ...
    expect(within(dialog).getByText(/Fuente 1 · M\+ Liga de Campeones --> Elcano/)).toBeInTheDocument();
    ...
    await screen.findByText('Fuente apartada; el segundo motor ya la está comprobando', { exact: false }).catch(() => null);
...
describe('«Encontrar canal»', () => {
  it('candidatos con su origen y disponibilidad, «Recordar» marcado y vínculo a mano', async () => {
    ...
    data.candidates[0] = candidate(1, { source: 'acestream', availability: 0.91, title: 'Zapping HD' });
    sessionStore.set({ ...sessionStore.get(), key: 'm:m1', kind: 'match', phase: 'choices', resolution: data, resolverOpen: true });
    ...
    expect(within(dialog).getByText('Hay varias coincidencias posibles. No reproduciremos ninguna sin que la confirmes.')).toBeInTheDocument();
    ...
    expect(within(dialog).getByRole('checkbox', { name: /Recordar mi elección para M\+ Liga de Campeones/ })).toBeChecked();
    expect(within(dialog).getByRole('button', { name: 'Copiar nombre del canal' })).toBeInTheDocument();
    // Nada suena hasta que se elige.
    expect(getPlayer().channel).toBeNull();
    fireEvent.change(within(dialog).getByLabelText('Content ID o enlace AceStream'), { target: { value: 'hola' } });
    await act(async () => {
      fireEvent.click(within(dialog).getByRole('button', { name: 'Vincular y reproducir' }));
    });
    expect(within(dialog).getByText('Introduce un Content ID o enlace AceStream válido de 40 caracteres.')).toBeInTheDocument();
  });
  ...
  it('sin candidatos: el texto depende de si el buscador estaba disponible', async () => {
    sessionStore.set({
      ...
      kind: 'match',
      phase: 'not_found',
      resolution: resolution(0, { status: 'not_found', candidate: null, scan: null, engineAvailable: false }),
      resolverOpen: true,
    });
    renderWithApp(<ResolverSheet />);
    const dialog = await screen.findByRole('dialog', { name: 'Encontrar canal' });
