...
| IPTV | `iptv-pestana.spec.ts` | La pestaña IPTV de Canales (docs/iptv.md §16.9, casos 14 a 20): con `?demo=1`, la pestaña a la derecha de Listas, recorrer y tocar, filtros combinados, buscar dentro, móvil sin desplazamiento lateral, teclado y axe; contra la pila, sin IPTV no hay pestaña, `iptvBrowse` interceptada (páginas, cursor y `stale`), el mensaje de los dos intentos; y con el proveedor falso, las categorías reales y tocar suena por hls.js (`@video`) y el primer «Guardar» con 502 se guarda al segundo intento. |
