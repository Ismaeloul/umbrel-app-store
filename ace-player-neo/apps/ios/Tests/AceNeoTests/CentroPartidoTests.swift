...
/// Política única de cambio de fuente del centro de partido (el `session.ts`
...
final class CentroPartidoTests: XCTestCase {
    ...
    @MainActor
    private func preparar() throws -> (AppModel, MotorFalso, CentroPartidoModelo) {
        ...
        let app = AppModel(entorno: entorno, reproductor: reproductor)
        let partido = try JSONDecoder().decode(FootballMatch.self, from: Data(partidoJSON.utf8))
        return (app, motor, CentroPartidoModelo(partido: partido, app: app))
    }
    ...
    @MainActor
    private func primeraImagen(_ app: AppModel, _ motor: MotorFalso) {
        motor.emitir(.listo)
    ...
    @MainActor
    private func agotar(_ id: String, _ app: AppModel, _ motor: MotorFalso) async {
