...
final class ReglasFuentesTests: XCTestCase {
    ...
    func testMedidorYPalabraComoEnLaWeb() {
        ...
        XCTAssertTrue(senal(entrada("a", sonda: .weak)) == (.floja, "Floja"))
        XCTAssertTrue(senal(entrada("a", sonda: .failed)) == (.sinSenal, "Sin señal"))
        XCTAssertTrue(senal(entrada("a", sonda: .checking)) == (.comprobando, "Comprobando"))
        XCTAssertTrue(senal(entrada("a", sonda: .queued)) == (.pendiente, "Pendiente"))
        XCTAssertTrue(senal(entrada("a", reportadaHasta: ahora.addingTimeInterval(9))) == (.sinSenal, "Reportada"))
        XCTAssertTrue(senal(entrada("a", disponibilidad: 0.8)) == (.ok, "80% disponible"))
        XCTAssertTrue(senal(entrada("a", disponibilidad: 30)) == (.floja, "30% disponible"))
        XCTAssertTrue(senal(entrada("a")) == (.pendiente, "Sin comprobar"))
    }
    ...
    func testCandidatoDeLaResolucion() throws {
        ...
        let entradas = resolucion.candidates.map { EntradaFuente($0) }
    ...
    func testFiltroDeLaBibliotecaSinAcentos() throws {
        let biblioteca = try JSONDecoder().decode(LibraryView.self, from: Fixtures.datos("v1/libraryGet.json"))
        XCTAssertEqual(FiltroBiblioteca.filtrar(biblioteca.web, texto: "laliga").map(\.title), ["M+ LaLiga"])
        XCTAssertEqual(FiltroBiblioteca.filtrar(biblioteca.web, texto: "DEPORTES").count, 2)
        XCTAssertEqual(FiltroBiblioteca.filtrar(biblioteca.web, texto: "").count, 2)
        XCTAssertEqual(FiltroBiblioteca.normalizar("Camión"), "camion")
    }
    ...
    func testMarcadorYMinuto() throws {
        ...
        XCTAssertEqual(Marcador.progreso(marcador, inicio: nil), 0.6, accuracy: 0.001)
        XCTAssertEqual(ColorEquipo.iniciales("Real Madrid"), "RM")
        XCTAssertEqual(ColorEquipo.iniciales("PSG"), "PS")
        let tono = ColorEquipo.tono("Real Madrid") * 360
        XCTAssertFalse((280...320).contains(tono), "Nada de violeta")
    }
}
