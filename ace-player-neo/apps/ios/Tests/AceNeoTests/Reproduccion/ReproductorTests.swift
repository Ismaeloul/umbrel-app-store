...
final class ReproductorTests: XCTestCase {
    ...
    @MainActor
    func testDetenerSueltaLaSesion() async throws {
        // Poda (fase 0.2): lo del mini (visibleEnMini, superficieGrande) salió del Reproductor; lo prueba
        // PresentacionReproductorTests (M3).
        let (reproductor, motor, servicio) = try preparar()
        await arrancar(reproductor, motor)
