...
/* Rescatado en la poda (fase 0.2, b-arquitectura §1.11 y §4.1.2) de `CentroPartidoModelo`
   ...
   ser `EntornoSesionFuentes`. El port completo de session.ts (una sola sesión con `conectar(_:)`,
   `entrarPartido`, `entrarCanal`, «Otras fuentes» calcado de la web, §0.0 punto 1) es de M3 (§2.6). */

/// Lo que la sesión de fuentes necesita de la app (§2.6). PROVISIONAL de la poda: solo lo que usa hoy.
/// La fase 0.3b le añade `haptica`, `hojas`, `reloj`, `visor` y `tiempoRealAbierto`, cambia `Avisos`
/// por el del contrato (§2.4.4) y lo conforma `ContenedorApp`.
@MainActor
protocol EntornoSesionFuentes: AnyObject {
...
final class SesionFuentes {
            ...
            for _ in 0..<240 {
                ...
                do {
                    ...
                    if fallos >= 3 {
                        ...
                        self.entorno.avisos.mostrar("El comprobador no responde; se muestran todas las fuentes")
    ...
    func pegar(_ texto: String, recordar: Bool) async -> Bool {
        guard let hash = ReglasFuentes.hashValido(texto) else {
            entorno.avisos.mostrar(ReglasFuentes.textoHashNoValido, tono: .error)
            return false
        }
        ...
        elegir(entrada)
        pegados += 1
        entorno.avisos.mostrar(existe ? "Reproduciendo la señal pegada" : "Señal añadida y reproduciendo", tono: .ok)
    ...
    func reportar(_ entrada: EntradaFuente, motivo: SourceReportReason) async {
        ...
        do {
            ...
            reportes += 1
            entorno.avisos.mostrar("Fuente reportada: se aparta y se vuelve a comprobar", tono: .ok)
        } catch {
            entorno.avisos.mostrar(APIError.desde(error).mensaje, tono: .error)
        }
    ...
    func corregir(_ entrada: EntradaFuente, correcto: Bool) async {
        ...
        do {
            ...
            entorno.avisos.mostrar(correcto ? "Anotado: es el canal correcto" : "Anotado: no se volverá a proponer", tono: .ok)
        } catch {
            entorno.avisos.mostrar(APIError.desde(error).mensaje, tono: .error)
        }
    }
