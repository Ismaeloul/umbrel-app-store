...
///
/// Vive fuera de la vista A PROPÓSITO (lo guarda `AppModel`): si la fuente se
...
@MainActor
@Observable
public final class CentroPartidoModelo {
    public let partido: FootballMatch
    public private(set) var resolucion: Resolution?
    public private(set) var entradas: [EntradaFuente] = []
    public private(set) var trabajo: ScanJob?
    ...
    public private(set) var automatico = true
    ...
    public var contexto: ContextoPartido {
        ContextoPartido(
    ...
    public var suenaAqui: Bool { reproductor.canal?.partido?.id == partido.id }

    /// La pantalla del partido entra o sale.
    public func vista(abierta: Bool) {
        let antes = vistaAbierta
        vistaAbierta = abierta
        if abierta && !antes { intentarArranqueAutomatico() }
    }

    public var terminado: Bool { sinComprobador || ReglasFuentes.terminado(trabajo) }

    public var progreso: Double { ReglasFuentes.progreso(trabajo, total: entradas.count) }

    /// Estado efectivo de cada entrada (con lo que hay en pantalla ahora).
    public func efectivos(ahora: Date = .now) -> [String: Efectivo] {
    ...
    public func cargar(rebuscar: Bool = false) async {
        ...
        do {
            let resolucion = try await app.entorno.api.enviar(
                API.resolver(
            ...
            fusionar(resolucion.candidates.map { EntradaFuente($0) })
    ...
    private func seguir(_ id: String) {
        ...
        tareaSeguimiento = Task { [weak self] in
            ...
            for _ in 0..<240 {
                ...
                do {
                    let trabajo = try await self.app.entorno.api.enviar(API.comprobacion(id: id))
                    ...
                    if fallos >= 3 {
                        ...
                        self.app.avisos.mostrar("El comprobador no responde; se muestran todas las fuentes")
    ...
    @discardableResult
    public func pegar(_ texto: String, recordar: Bool) async -> Bool {
    ...
    public func reportar(_ entrada: EntradaFuente, motivo: SourceReportReason) async {
    ...
    public func corregir(_ entrada: EntradaFuente, correcto: Bool) async {
        ...
        do {
            _ = try await app.entorno.api.enviar(API.correccionFuente(cuerpo))
            if let indice = entradas.firstIndex(where: { $0.id == entrada.id }) {
                entradas[indice].aprendida = correcto ? .correct : .incorrect
            }
            app.avisos.mostrar(correcto ? "Anotado: es el canal correcto" : "Anotado: no se volverá a proponer", tono: .ok)
        } catch {
            app.avisos.mostrar(APIError.desde(error).mensaje, tono: .error)
        }
    }
