...
**Pestaña IPTV en Canales (26-sep-2026, para la 0.8.2; diseño en `iptv/pestana`; contrato y web hechos en
`iptv/pestana-web`, servidor en `iptv/pestana-servidor`):** recorrer la IPTV por las categorías del proveedor, con
buscador y filtros de país, idioma, tipo, deporte y calidad calculados en el servidor, y el arreglo del 502 del primer
«Guardar». El anexo §16 lo diseña y **manda** sobre §14.7 (canales listados sin buscar) y sobre «nunca lleva grupo» de
§14.2. Lo que ya está y lo que falta, en §16.13.
...
### 16.14 Estado: la web y la unión (26-sep, rama `iptv/pestana-web`, con `iptv/pestana-servidor` y `rediseno/iptv`)
...
**Unión con «todo desbloqueado + variantes»** (`rediseno/iptv` hasta `98073f9`, ahora §17):
- El canal tocado y los respaldos del relé usan lo de §17 (`tappedCandidates` con `groupMatch` y `catalog.channelOf`,
  `relayVariants`): el país ya lo respeta el canal de §17, así que `rowVariants` de la pestaña queda solo para las
  filas de `browse.ts`. Si una fila de la pestaña saca su país de la categoría (y §17 no lo ve en el nombre), al
  tocarla arranca la mejor variante de su canal de §17: puede no coincidir en casos raros (riesgo pequeño, anotado).
- `isAdultChannel` sale del buscador (§17 lo quita) y vive en `facets.ts`, que lo sigue usando para el tipo «Adultos».
- `CatalogEntry` lleva las dos cosas: `tvgCountry`/`tvgLanguage` (pestaña) y `mirror`/`keepMirror` (§17).
- La fila: `ChannelRow.tags` de §17 (etiquetas detrás del subtítulo, con su CSS); la pestaña le pasa solo las
  calidades (el país ya va en el subtítulo), así se ve igual que la del buscador.
- Proveedor falso con los dos: `grande` y `fallarPrimera` (pestaña) y las 5 variantes de «DAZN 1» y los canales de
  otros países (§17). Los tests de la pestaña se ajustan a él (orden de categorías con «DE | SPORT» y «FR | SPORT»,
  «DAZN 1» con 4K, 1080p, 720p y SD, y el canal tocado de España arranca por su 1080p).

**Falta:** `docs/comportamientos.md` y la tabla de §12.1 al cerrar; publicar (0.8.2) cuando Isma lo diga.
...
> Numeración: en la rama `rediseno/iptv` este anexo era el §16; al unirlo con la pestaña IPTV (que ya era el §16) pasa a §17. Los comentarios del código de las variantes que dicen «§16» se refieren a este.
