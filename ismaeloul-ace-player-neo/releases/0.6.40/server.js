...
const SCANNER_JOB_TTL_MS = 15 * 60 * 1000;
const SCANNER_GOOD_TTL_MS = 3 * 60 * 1000;
const SCANNER_BAD_TTL_MS = 45 * 1000;
...
const SEMANTIC_MIN_SIMILARITY = 0.82;
...
function semanticCatalog(requestedChannels, programChannels) {
  ...
  for (const raw of [...requestedChannels, ...(Array.isArray(programChannels) ? programChannels : [])]) {
    ...
    const requested = requestedChannels.some((channel) => channelMatchScore(channel, name) >= 92);
...
async function applySemanticCandidateScores(requestedChannels, candidates, programChannels, options = {}) {
  ...
  const scored = candidates.map((candidate) => {
    ...
    return {
      ...candidate,
      score: aiScore,
      matchedChannel: requestedChannels.find((channel) => channelMatchScore(channel, bestRequested.channel) >= 92)
        || requestedChannels[0],
      soloFamilia: false,
      semantic: true,
      semanticSimilarity: Number(bestRequested.similarity.toFixed(4)),
...
function classifyScannerEvidence(evidence, minBytes = SCANNER_SAMPLE_BYTES) {
  ...
  if (mediaResponse && bytes >= minBytes) return { state: "working", reason: "media_data" };
  if (mediaResponse && (bytes >= 16 * 1024 || downloadedDelta >= 16 * 1024 || (peers > 0 && speedDown > 0))) {
...
async function probeAceCandidate(candidate, options = {}) {
  ...
  try {
    ...
    const sampled = await sample(playbackPath, remaining, minBytes);
    ...
    const classification = classifyScannerEvidence({ ...sampled, ...finalStats, downloadedDelta }, minBytes);
    return {
      ...classification,
...
function pruneScannerState(now = Date.now()) {
  ...
  for (const [id, cached] of scannerCache) {
    if (now - cached.cachedAt > SCANNER_GOOD_TTL_MS) scannerCache.delete(id);
  }
}
...
function scannerJobPayload(job) {
  const candidates = job.candidates.map((item) => ({
    id: item.id,
    state: item.state === "retry" ? "queued" : item.state,
...
function enqueueScannerJob(job) {
  if (!job || job.enqueued || job.status === "cancelled" || job.status === "complete") return;
  if (!job.candidates.some((item) => item.state === "queued" || item.state === "retry")) return;
  job.enqueued = true;
  scannerQueue.push(job.id);
...
async function drainScannerQueue() {
  ...
  try {
    while (scannerQueue.length) {
      ...
      /* Primero se da una oportunidad a TODAS. Los fallos se reintentan solo
         cuando ya se ha recorrido la lista, para que una fuente lenta no
         retrase las alternativas que pueden estar vivas. */
      const candidate = job.candidates.find((item) => item.state === "queued")
        || job.candidates.find((item) => item.state === "retry");
      if (!candidate) {
        job.status = "complete";
      ...
      const cached = scannerCacheHit(candidate.id);
      if (cached) {
        ...
        if (result.state === "failed" && candidate.attempts < 2) {
      ...
      job.updatedAt = Date.now();
      if (job.status === "cancelled") continue;
      if (job.candidates.some((item) => item.state === "queued" || item.state === "retry")) enqueueScannerJob(job);
...
function createScannerJob(candidates, clientValue = "") {
  ...
  for (const item of Array.isArray(candidates) ? candidates : []) {
    ...
    if (!id || seen.has(id)) continue;
    seen.add(id);
    normalized.push({ id, ih: item.ih === true, state: "queued", attempts: 0 });
  ...
  for (const candidate of job.candidates) {
    const cached = scannerCacheHit(candidate.id, now);
    if (cached) Object.assign(candidate, cached, { cached: true });
  }
  if (!job.candidates.some((item) => item.state === "queued" || item.state === "retry")) job.status = "complete";
  scannerJobs.set(job.id, job);
  if (clientKey) scannerClients.set(clientKey, job.id);
  enqueueScannerJob(job);
  return {
    id: job.id,
...
function libraryResolutionCandidates(state, channels, minimumScore = LIBRARY_MIN_SCORE) {
  const output = [];
  const seen = new Set();
...
function resolutionTier(candidate) {
  if (candidate.score >= 92) return 2;   // es ese canal, sin duda
...
function mergeResolutionCandidates(candidates) {
  ...
  const hayExacto = todos.some((candidate) =>
    candidate.source !== "saved" && !candidate.soloFamilia && candidate.score >= 92);
  const vivas = todos
    .filter((candidate) => {
  ...
  const GRUPOS = [
    (c) => c.source === "saved",
    (c) => c.source === "m3u",
    (c) => c.source === "favorites",
    (c) => c.source === "history",
    (c) => c.source === "acestream",
    () => true,
  ];
  const salida = [];
...
function minimumResolutionScore(candidate, semanticUsed = false) {
  if (candidate.source === "saved") return 92;
...
async function resolveFootballChannel(state, values, search = searchAceStreams, options = {}) {
  ...
  const local = libraryResolutionCandidates(state, channels, 0);
  ...
  const qualified = semanticResult.candidates
    .filter((candidate) => candidate.score >= minimumResolutionScore(candidate, semanticResult.used));
  const candidates = mergeResolutionCandidates([...bindings, ...qualified]);
  const ai = {
  ...
  if (!candidates.length) {
    return { success: true, status: "not_found", channels, checked, candidates: [], engineAvailable, ai, program };
  }
  ...
  if (elegido) {
    return { success: true, status: "found", channels, checked, candidate: elegido, candidates, engineAvailable, ai, program };
  }
  return { success: true, status: "choices", channels, checked, candidates, engineAvailable, ai, program };
}
  ...
  try {
    ...
    if (req.url === "/api/football/resolve" || req.url.startsWith("/api/football/resolve?")) {
      ...
      const result = await resolveFootballChannel(
        readState(), announcedChannels, searchAceStreams, { program },
      );
      result.scan = createScannerJob(result.candidates, url.searchParams.get("client"));
