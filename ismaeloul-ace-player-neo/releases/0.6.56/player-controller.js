...
(function exposeNeoPlayer(root, factory) {
  ...
  function resolveLiveTarget(range, preferredTarget, safetySeconds = 0) {
    if (!range || !Number.isFinite(range.start) || !Number.isFinite(range.end)
      || !Number.isFinite(range.duration) || range.duration <= 0) return null;
    ...
    const preferred = Number.isFinite(preferredTarget) ? preferredTarget : safeEdge;
  ...
  class NeoPlayerController {
    ...
    async goLive(targetInfo) {
      ...
      /* Pulsar de nuevo mientras el búfer técnico se recompone no debe lanzar
         otro seek y reiniciar el mismo ciclo. Ya seguimos el borde LIVE. */
      if (alreadyFollowing) return this.requestPlay("live-following");
  ...
  return { NeoPlayerController, clamp, readSeekWindow, resolveLiveTarget, waitForMedia };
});
