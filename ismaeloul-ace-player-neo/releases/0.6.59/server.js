...
/* Copia del estado anterior a cada escritura. Si state.json aparece ilegible
   -disco lleno, apagon a mitad de escritura- se recupera de aqui en vez de
   arrancar con favoritos, historial y aprendizaje a cero. */
...
/* Secreto compartido con engine-control (el compose pone APP_SEED en los dos).
   Sin el, cualquier contenedor del NAS podia reiniciar el motor a media
   emision. Vacio = sin comprobacion, para no romper instalaciones antiguas. */
const ENGINE_CONTROL_TOKEN = String(process.env.ENGINE_CONTROL_TOKEN || "").trim().slice(0, 200);
/* AUTO_SYNC=false apaga la sincronizacion periodica de directorios: los tests
   cargan este modulo y no tienen por que salir a internet. */
const AUTO_SYNC_ENABLED = process.env.AUTO_SYNC !== "false";
...
/* El iPhone empieza por donde le dice la lista: con un solo segmento arranca
   pegado al directo y sin colchon, y el primer bache del P2P lo para. Se
   espera a tener REMUX_READY_SECONDS de video; si en REMUX_READY_WAIT_MS no
   han llegado, se sirve lo que haya. */
const REMUX_READY_SECONDS = 6;
const REMUX_READY_WAIT_MS = 20 * 1000;
const REMUX_START_TIMEOUT_MS = 45 * 1000;
const REMUX_STALE_MS = 15 * 1000;
...
/* Pasarelas IPFS publicas intercambiables: si una responde 429 o 5xx al
   directorio, se prueba la otra con la misma ruta /ipns/... o /ipfs/...
   Desde septiembre de 2026 ya no sirven ficheros; quedan de respaldo. */
const IPFS_GATEWAYS = ["ipfs.io", "dweb.link"];
/* Lo que sigue abierto para bajar de IPFS sin pasarela publica: el enrutado
   delegado (resuelve nombres IPNS) y la pasarela trustless (bloques en CAR). */
const IPFS_DELEGATED_ROUTING = process.env.IPFS_DELEGATED_ROUTING || "https://delegated-ipfs.dev";
const IPFS_TRUSTLESS_GATEWAY = process.env.IPFS_TRUSTLESS_GATEWAY || "https://trustless-gateway.link";
/* Hasta 8 rotulos por partido, no 4: futbolenlatv anuncia cinco o seis y el
   ultimo podia ser justo el unico que tienes en la biblioteca. */
const MAX_RESOLUTION_CHANNELS = 8;
/* Cada dia el marcador crea una clave liga@rango nueva; sin poda, el mapa
   crecia para siempre. */
const SCORES_CACHE_STALE_MS = 24 * 60 * 60 * 1000;
...
const SCANNER_PROBE_TIMEOUT_MS = Math.min(30000, Math.max(6000, Number.parseInt(process.env.ACESTREAM_SCANNER_TIMEOUT_MS, 10) || 24000));
...
/* Cuanto se mantiene abierta la sesion de prueba. El motor pide de golpe a
   los pares los ~10 MB de colchon inicial, a veces a 4 MB/s, y solo despues
   se ve lo que el enjambre da de verdad. Se mide la entrada del motor -su
   contador "downloaded", no la lectura HTTP, que sale a rafagas- entre la
   mitad y el final de esta ventana. Medido: la señal mala baja a 1,4 Mbit/s
   en la segunda mitad; la buena se queda en 8. */
const SCANNER_SUSTAIN_MS = Math.min(15000, Math.max(4000, Number.parseInt(process.env.ACESTREAM_SCANNER_SUSTAIN_MS, 10) || 12000));
// Por debajo de esta fraccion del bitrate del canal la señal se para al rato.
const SCANNER_STARVED_RATIO = 0.85;
/* 10 min, no 3: una señal que iba hace cinco minutos casi seguro sigue
   yendo, y el reproductor la valida igualmente al arrancar. Con 3 min la
   precarga caducaba antes del saque y se volvia a probar todo al pulsar. */
const SCANNER_GOOD_TTL_MS = 10 * 60 * 1000;
const SCANNER_BAD_TTL_MS = SCANNER_RETRY_DELAY_MS;
/* Lo que el reproductor ve en el motor principal manda sobre la muestra del
   comprobador, que ve peor el enjambre (no recibe pares entrantes): durante
   este rato ninguna prueba lo pisa. Mientras se ve un canal, la web lo
   renueva cada dos minutos. */
const PLAYER_VERDICT_HOLD_MS = 3 * 60 * 1000;
// Fallos que hablan del video y no de la red: no se suavizan
const SCANNER_HARD_FAILURES = new Set(["unsupported_codec", "no_video"]);
...
/* 24, no 96: medido en el NAS, 100 nombres tardan 6,9 s en embeddinggemma y
   el tope por peticion era 6,5 s, asi que el primer lote se pasaba siempre. */
const OLLAMA_EMBED_BATCH = 24;
...
/* Con el tope de sesiones alcanzado se desaloja primero la que ya no tiene
   espectadores -sin clientes o con ffmpeg terminado-, la mas antigua entre
   ellas. Antes se mataba la mas antigua sin mirar, y un cuarto iPhone dejaba
   a oscuras a uno que estaba viendo el partido. Si todas estan vivas, null:
   el que llega espera. */
function elegirSesionRemuxADesalojar(sessions) {
  let elegida = null;
  for (const [key, session] of sessions) {
    const libre = session.exited || !session.clients || session.clients.size === 0;
    if (!libre) continue;
    if (!elegida || session.lastAccess < elegida.lastAccess) elegida = { key, lastAccess: session.lastAccess };
  }
  return elegida ? elegida.key : null;
}

/* Segmentos y segundos de video que ya tiene la lista de un remux. */
function remuxPlaylistStats(file) {
  let text;
...
/* Un ffmpeg que lleva REMUX_STALE_MS sin tocar la lista esta atascado (el
   motor dejo de mandar datos). Reutilizarlo en un reintento devolvia al
   iPhone la misma lista congelada: se arranca uno nuevo. */
function remuxStalled(session, now = Date.now()) {
  if (now - (session.startedAt || 0) < REMUX_START_TIMEOUT_MS) return false;
  try { return now - fs.statSync(path.join(session.dir, "index.m3u8")).mtimeMs > REMUX_STALE_MS; }
  catch { return true; }
}

/* Devuelve la sesion y la ficha de este enganche. Cada peticion a /api/remux
   estrena ficha para el dispositivo; un /api/remux/stop con una ficha vieja
   -el de un reintento que llega tarde- ya no desengancha al que acaba de
   volver a entrar. */
function ensureRemux(idParam, id, deviceId = "") {
  const key = id.toLowerCase();
  const token = crypto.randomBytes(8).toString("hex");
  let s = remuxSessions.get(key);
  if (s && !s.exited && s.idParam === idParam && !remuxStalled(s)) {
    s.lastAccess = Date.now();
    if (deviceId) s.clients.set(deviceId, token);
    return { session: s, token };
  }
  if (s) remuxCleanup(key);
  while (remuxSessions.size >= MAX_REMUX_SESSIONS) {
    const victima = elegirSesionRemuxADesalojar(remuxSessions);
    if (!victima) {
      const error = new Error("remux_busy");
      error.statusCode = 503;
      throw error;
    }
    remuxCleanup(victima);
  }
  ...
  const proc = spawn("ffmpeg", [
    ...
    /* Un corte del motor ya no mata la sesion. Sin estas opciones ffmpeg se
       quedaba colgado para siempre si el P2P dejaba de mandar datos, o
       terminaba al primer cierre de la conexion. Ahora, tras 20 s sin datos
       o al cerrarse la conexion, vuelve a pedir el stream por su cuenta y la
       lista sigue creciendo sin que el iPhone tenga que empezar de cero. */
    ...
    /* Segmentos de 2 s (o de un GOP, si es mas largo) en una ventana de 30 s:
       el iPhone arranca unos segmentos por detras del directo, con colchon, y
       tras un bache aun encuentra en la lista lo que le falta.
       omit_endlist: si ffmpeg termina, la lista no se da por acabada. Antes el
       iPhone llegaba al "final", se quedaba en pausa y nada lo reanudaba. */
  ...
  s = {
    proc, dir, idParam,
    clients: new Map(deviceId ? [[deviceId, token]] : []),
    startedAt: Date.now(), lastAccess: Date.now(), exited: false, exitCode: null,
  };
...
/* Calentar el indice no puede ser todo o nada: un lote que se pasa de tiempo
   -el primero, con el modelo frio- tiraba el trabajo entero y se volvia a
   intentar igual media hora despues (era el "ollama_timeout" del registro).
   Aqui un fallo solo deja ese lote para la siguiente vez; lo demas queda en
   cache y sirve ya. */
async function semanticWarmEmbeddings(values, options = {}) {
  const cache = options.cache || semanticEmbeddingCache;
  const embed = options.embed || ollamaEmbedBatch;
  ...
  return {
    ...
    failed,
    error: lastError ? (lastError.message || "ollama_unavailable") : null,
  };
}
...
function normalizeWebSource(source, index = 0, fallbackStreams = [], fallbackSyncedAt = null) {
  ...
  return {
    ...
    // motivo corto del ultimo fallo ("http_429", "fetch_timeout"...) para que
    // la tarjeta del directorio diga por que, no solo que fallo
    lastError: typeof source?.lastError === "string"
      ? source.lastError.toLowerCase().replace(/[^a-z0-9_]/g, "").slice(0, 40) || null
      : null,
  };
}
...
function sourceSummaries(sources) {
  return sources.map(({ id, name, url, type, streams, syncedAt, lastErrorAt, lastError }) => ({
    id, name, url, type, count: streams.length, syncedAt, lastErrorAt, lastError: lastErrorAt ? lastError : null,
  }));
}
...
/* Lee y parsea state.json. Si esta ilegible no se devuelve el estado vacio
   sin mas: se aparta el fichero con marca de tiempo -para poder mirarlo-, se
   avisa en el log y se intenta la copia de la escritura anterior. Solo si
   tampoco hay copia se arranca de cero. */
function readStateJson() {
  try {
    return JSON.parse(fs.readFileSync(STATE_FILE, "utf8"));
  } catch (error) {
    const stamp = new Date().toISOString().replace(/[:.]/g, "-");
    const apartado = `${STATE_FILE}.corrupt-${stamp}`;
    try { fs.renameSync(STATE_FILE, apartado); } catch {}
    console.error(`[estado] state.json ilegible (${error.message}); apartado en ${path.basename(apartado)}`);
    let backup;
    try { backup = JSON.parse(fs.readFileSync(STATE_BACKUP_FILE, "utf8")); }
    catch { throw new Error("state_unreadable"); }
    try { fs.copyFileSync(STATE_BACKUP_FILE, STATE_FILE); } catch {}
    console.error("[estado] recuperado desde state.json.bak");
    return backup;
  }
}
...
function writeState(nextState) {
  ...
  // el anterior pasa a ser la copia de seguridad; el nuevo entra de forma atomica
  try { fs.renameSync(STATE_FILE, STATE_BACKUP_FILE); } catch {}
...
// options.binary devuelve un Buffer (bloques IPFS); options.accept cambia la
// cabecera Accept para los formatos de IPFS
async function fetchText(url, redirects = 0, visited = new Set(), deadline = Date.now() + FETCH_TOTAL_TIMEOUT_MS, maxBytes = MAX_BODY, options = {}) {
  ...
  return new Promise((resolve, reject) => {
    ...
    const req = client.get(parsed, {
      ...
      headers: {
        "User-Agent": "AcePlayerNeo/0.6.59",
        Accept: options.accept || "application/json,text/plain,text/html,application/x-mpegURL,*/*;q=0.2",
      ...
      if (response.statusCode >= 300 && response.statusCode < 400 && response.headers.location) {
        ...
        clearTimeout(totalTimer);
        fetchText(nextUrl, redirects + 1, visited, deadline, maxBytes, options)
          .then((value) => finish(resolve, value), (error) => finish(reject, error));
        return;
      }
      if (response.statusCode < 200 || response.statusCode >= 300) {
        response.resume();
        // "http_429" en vez de un "fetch_failed" generico: el motivo llega
        // hasta la tarjeta del directorio y decide si probar otra pasarela
        finish(reject, new Error(`http_${response.statusCode}`));
        return;
      }
      ...
      response.on("end", () => {
        ...
        finish(resolve, options.binary ? body : body.toString("utf8"));
      });
...
/* Misma ruta en la otra pasarela IPFS, o null si la URL no es de ninguna. */
function alternateGatewayUrl(url) {
  let parsed;
  try { parsed = new URL(url); } catch { return null; }
  if (parsed.protocol !== "https:") return null;
  const index = IPFS_GATEWAYS.indexOf(parsed.hostname.toLowerCase());
  if (index < 0 || !/^\/(ipfs|ipns)\//.test(parsed.pathname)) return null;
  parsed.hostname = IPFS_GATEWAYS[(index + 1) % IPFS_GATEWAYS.length];
  return parsed.toString();
}

function esFalloDePasarela(error) {
  return /^http_(429|5\d\d)$/.test(String(error?.message || ""));
}

/* ---------- IPFS sin pasarela publica ----------
   Desde septiembre de 2026 ipfs.io, dweb.link y w3s.link ya no sirven
   ficheros: contestan 429 con "switching to a service worker gateway only".
   Un directorio en /ipns/<nombre>/<ruta> se baja ahora como lo hace ese
   service worker en el navegador:
   1. el enrutado delegado da el registro IPNS -> /ipfs/<cid>[/ruta];
   2. la pasarela trustless entrega en un CAR los bloques de esa ruta, y aqui
      se recorre el directorio y se junta el fichero.
   Cada bloque se comprueba contra su hash, asi que un CAR alterado no cuela.
   La firma del registro IPNS no se verifica: se confia en el enrutado
   delegado igual que antes se confiaba en ipfs.io. */

function ipfsVarint(buffer, position) {
  let value = 0;
  let shift = 0;
  let byte;
  do {
    if (position >= buffer.length || shift > 49) throw new Error("ipfs_bad_data");
...
/* Campos de un mensaje protobuf como [numero, valor]. Solo hacen falta varint
   y bytes: es todo lo que usan dag-pb, UnixFS y el registro IPNS. */
function ipfsProtobuf(buffer) {
  const fields = [];
  let position = 0;
  while (position < buffer.length) {
    ...
    if (wire === 0) {
      ...
      if (position + length > buffer.length) throw new Error("ipfs_bad_data");
      ...
      throw new Error("ipfs_bad_data");
    }
  }
  return fields;
}

/* CBOR minimo: el campo "data" del registro IPNS v2 es un mapa CBOR. */
function ipfsCbor(buffer) {
  let position = 0;
  const sizes = { 24: 1, 25: 2, 26: 4, 27: 8 };
  const argument = (info) => {
    if (info < 24) return info;
    const size = sizes[info];
    if (!size || position + size > buffer.length) throw new Error("ipfs_bad_record");
    let value = 0;
    for (let index = 0; index < size; index += 1) value = value * 256 + buffer[position++];
    return value;
  };
  const item = (depth) => {
    if (depth > 8 || position >= buffer.length) throw new Error("ipfs_bad_record");
    const first = buffer[position++];
    const major = first >> 5;
    const info = first & 31;
    if (major === 7) {
      if (sizes[info]) position += sizes[info];
    ...
    if (major === 2 || major === 3) {
      if (position + value > buffer.length) throw new Error("ipfs_bad_record");
    ...
    throw new Error("ipfs_bad_record");
  };
  return item(0);
}
...
function ipfsBase58(text) {
  const bytes = [0];
  for (const char of text) {
    let carry = IPFS_BASE58.indexOf(char);
    if (carry < 0) throw new Error("ipfs_bad_cid");
...
function ipfsBase32(text) {
  ...
  for (const char of text) {
    ...
    if (index < 0) throw new Error("ipfs_bad_cid");
...
/* CID binario -> { codec, hash, digest, end }. Un CIDv0 es solo el multihash
   sha2-256 de un nodo dag-pb. */
function ipfsReadCid(buffer, start = 0) {
  if (buffer[start] === 0x12 && buffer[start + 1] === 0x20) {
    return { codec: 0x70, hash: 0x12, digest: buffer.subarray(start + 2, start + 34), end: start + 34 };
  }
  let position = start;
  let version;
  let codec;
  let hash;
  let length;
  [version, position] = ipfsVarint(buffer, position);
  if (version !== 1) throw new Error("ipfs_bad_cid");
  [codec, position] = ipfsVarint(buffer, position);
  [hash, position] = ipfsVarint(buffer, position);
  [length, position] = ipfsVarint(buffer, position);
  if (position + length > buffer.length) throw new Error("ipfs_bad_cid");
  return { codec, hash, digest: buffer.subarray(position, position + length), end: position + length };
}

function ipfsCidFromText(text) {
  ...
  throw new Error("ipfs_bad_cid");
}

const ipfsBlockKey = (cid) => `${cid.hash}:${cid.digest.toString("hex")}`;

/* Bloques de un CAR v1, comprobados uno a uno contra su hash. */
function ipfsCarBlocks(car) {
  let [headerLength, position] = ipfsVarint(car, 0);
  position += headerLength;
  const blocks = new Map();
  while (position < car.length) {
    let length;
    [length, position] = ipfsVarint(car, position);
    if (position + length > car.length) throw new Error("ipfs_bad_data");
    const section = car.subarray(position, position + length);
    position += length;
    const cid = ipfsReadCid(section);
    const block = section.subarray(cid.end);
    if (cid.hash === 0x12) {
      if (!crypto.createHash("sha256").update(block).digest().equals(cid.digest)) throw new Error("ipfs_bad_block");
    } else if (cid.hash !== 0x00) {
      throw new Error("ipfs_unsupported_hash");
    }
...
/* Nodo dag-pb con su UnixFS: enlaces con nombre, tipo y datos en linea. */
function ipfsNode(block) {
  const links = [];
  ...
  for (const [field, value] of ipfsProtobuf(block)) {
    if (field === 2 && Buffer.isBuffer(value)) {
      ...
      for (const [linkField, linkValue] of ipfsProtobuf(value)) {
        ...
        else if (linkField === 2 && Buffer.isBuffer(linkValue)) link.name = linkValue.toString("utf8");
      }
      if (link.cid) links.push(link);
...
function ipfsBlock(blocks, cid) {
  if (cid.hash === 0x00) return cid.digest;
  const block = blocks.get(ipfsBlockKey(cid));
  if (!block) throw new Error("ipfs_missing_block");
  return block;
}

function ipfsWalk(blocks, root, segments) {
  let cid = root;
  for (const segment of segments) {
    if (cid.codec !== 0x70) throw new Error("ipfs_not_found");
    const node = ipfsNode(ipfsBlock(blocks, cid));
    // Los directorios enormes van repartidos en un HAMT: no es el caso de
    // ningun directorio conocido y se deja al respaldo de la pasarela.
    if (node.type === 5) throw new Error("ipfs_hamt_unsupported");
    const link = node.links.find((entry) => entry.name === segment);
    if (!link) throw new Error("ipfs_not_found");
    cid = link.cid;
  }
  return cid;
}

function ipfsReadFile(blocks, cid, limit, depth = 0) {
  if (depth > 16) throw new Error("ipfs_bad_data");
  const block = ipfsBlock(blocks, cid);
  if (cid.codec === 0x55) return block;
  if (cid.codec !== 0x70) throw new Error("ipfs_unsupported_codec");
  const node = ipfsNode(block);
  if (node.type !== 2 && node.type !== 0) throw new Error("ipfs_not_file");
...
/* { kind, name, segments } si la URL es de una pasarela IPFS, estilo ruta
   (https://ipfs.io/ipns/<nombre>/lista.m3u) o subdominio
   (https://<nombre>.ipns.dweb.link/lista.m3u); null si no. */
function ipfsUrlParts(url) {
  let parsed;
  try { parsed = new URL(url); } catch { return null; }
  if (parsed.protocol !== "https:") return null;
  const segmentsOf = (pathname) => pathname.split("/").filter(Boolean).map((segment) => {
    try { return decodeURIComponent(segment); } catch { return segment; }
  });
  const byPath = parsed.pathname.match(/^\/(ipfs|ipns)\/([^/]+)(\/.*)?$/);
  if (byPath) return { kind: byPath[1], name: byPath[2], segments: segmentsOf(byPath[3] || "") };
  const bySubdomain = parsed.hostname.toLowerCase().match(/^([a-z0-9]+)\.(ipfs|ipns)\.[a-z0-9.-]+$/);
  if (bySubdomain) return { kind: bySubdomain[2], name: bySubdomain[1], segments: segmentsOf(parsed.pathname) };
  return null;
}

/* Valor de un nombre IPNS: /ipfs/<cid>[/ruta] o /ipns/<otro>. Un nombre con
   punto es un dominio con DNSLink y va por su registro TXT. */
async function ipfsResolveName(name) {
  if (name.includes(".")) {
    let records;
    try { records = await dns.promises.resolveTxt(`_dnslink.${name}`); }
    catch { throw new Error("dns_failed"); }
    const link = records.map((parts) => parts.join("")).find((text) => text.startsWith("dnslink="));
    if (!link) throw new Error("ipfs_bad_record");
    return link.slice("dnslink=".length);
  }
  const record = await fetchText(
    `${IPFS_DELEGATED_ROUTING}/routing/v1/ipns/${encodeURIComponent(name)}`,
    0, new Set(), Date.now() + FETCH_TOTAL_TIMEOUT_MS, 64 * 1024,
    { accept: "application/vnd.ipfs.ipns-record", binary: true },
  );
  let value = null;
  for (const [field, content] of ipfsProtobuf(record)) {
    // El campo 9 (datos CBOR de la v2) manda sobre el 1, que es el de la v1
    if (field === 9 && Buffer.isBuffer(content)) {
      const data = ipfsCbor(content);
      if (data && data.Value !== undefined) value = data.Value;
    } else if (field === 1 && value === null && Buffer.isBuffer(content)) {
      value = content;
    }
  }
  const text = Buffer.isBuffer(value) ? value.toString("utf8") : String(value || "");
  if (!text) throw new Error("ipfs_bad_record");
  return text;
}

async function fetchIpfsDirectory(url) {
  const parts = ipfsUrlParts(url);
  if (!parts) throw new Error("bad_url");
  ...
  for (let hops = 0; kind === "ipns"; hops += 1) {
    if (hops >= 3) throw new Error("ipfs_bad_record");
    const target = (await ipfsResolveName(name)).match(/^\/(ipfs|ipns)\/([^/]+)(\/.*)?$/);
    if (!target) throw new Error("ipfs_bad_record");
    kind = target[1];
    name = target[2];
  ...
  const car = await fetchText(
    `${IPFS_TRUSTLESS_GATEWAY}/ipfs/${encodeURIComponent(name)}${route ? `/${route}` : ""}?format=car&dag-scope=entity`,
    0, new Set(), Date.now() + FETCH_TOTAL_TIMEOUT_MS, MAX_BODY * 2,
    { accept: "application/vnd.ipld.car", binary: true },
  );
  const blocks = ipfsCarBlocks(car);
  return ipfsReadFile(blocks, ipfsWalk(blocks, root, segments), MAX_BODY).toString("utf8");
}

/* Descarga de un directorio. Si la URL es de IPFS se baja sin pasarela
   publica; si eso falla (o no es IPFS) se usa la URL tal cual. */
async function fetchDirectoryText(url) {
  if (!ipfsUrlParts(url)) return fetchDirectoryFromGateway(url);
  try {
    return await fetchIpfsDirectory(url);
  } catch (error) {
    console.warn(`[directorio] IPFS sin pasarela: ${motivoDeFallo(error)}; probando la pasarela publica`);
    try { return await fetchDirectoryFromGateway(url); }
    catch { throw error; }
  }
}

/* Como fetchText -mismo filtro de IPs privadas- pero si una pasarela IPFS
   publica se satura (429) o cae (5xx) se prueba una vez la otra con la misma
   ruta. Si tampoco, se conserva el error original. */
async function fetchDirectoryFromGateway(url) {
  try {
    return await fetchText(url);
  } catch (error) {
    const alternativa = esFalloDePasarela(error) ? alternateGatewayUrl(url) : null;
    if (!alternativa) throw error;
    console.warn(`[directorio] ${new URL(url).hostname} respondio ${error.message}; probando ${new URL(alternativa).hostname}`);
    try { return await fetchText(alternativa); }
    catch { throw error; }
  }
}

/* Motivo de fallo apto para guardar y enseñar: solo los nombres cortos que
   genera este servidor; cualquier otra cosa (errores de red de Node) queda
   como fetch_failed. */
function motivoDeFallo(error) {
...
function rememberFootballProgramming(payload) {
  ...
  footballProgramming.warmPending = semanticWarmEmbeddings(texts)
    .then((result) => {
      if (!result.failed) { footballProgramming.signature = signature; return; }
      // firma vacia: la siguiente agenda vuelve a pedir solo lo que falta
      footballProgramming.signature = "";
      console.error(`[ia-programacion] indice parcial: ${result.failed} de ${result.requested} nombres sin vector (${result.error})`);
    })
    .catch((error) => {
      footballProgramming.signature = "";
...
/* Lee PAT, PMT y PCR de un trozo de transport stream. Da el codec sin
   depender de ffprobe y, con dos PCR y los bytes entre medias, el bitrate
   real del canal: es contra lo que hay que medir el caudal del enjambre. */
const TS_VIDEO_TYPES = { 0x01: "mpeg1video", 0x02: "mpeg2video", 0x10: "mpeg4", 0x1b: "h264", 0x24: "hevc" };
const TS_AUDIO_TYPES = { 0x03: "mp2", 0x04: "mp2", 0x0f: "aac", 0x11: "aac_latm", 0x81: "ac3", 0x87: "eac3" };

function analyzeTransportStream(buffer) {
  const out = { videoCodec: "", audioCodecs: [], streamKbps: 0, pcrSpanMs: 0, packets: 0 };
  if (!Buffer.isBuffer(buffer) || buffer.length < 188 * 3) return out;
  let offset = 0;
  while (offset < buffer.length - 376 && !(buffer[offset] === 0x47 && buffer[offset + 188] === 0x47 && buffer[offset + 376] === 0x47)) offset += 1;
  if (offset >= buffer.length - 376) return out;
  const pmtPids = new Set();
  const streams = new Map();
  let firstPcr = null, lastPcr = null, firstPcrOffset = 0, lastPcrOffset = 0;
  for (let p = offset; p + 188 <= buffer.length; p += 188) {
    if (buffer[p] !== 0x47) break;
    out.packets += 1;
    const pid = ((buffer[p + 1] & 0x1f) << 8) | buffer[p + 2];
    const pusi = (buffer[p + 1] & 0x40) !== 0;
    const adaptation = (buffer[p + 3] >> 4) & 3;
    let q = p + 4;
    if (adaptation & 2) {
      const length = buffer[p + 4];
      if (length >= 7 && (buffer[p + 5] & 0x10)) {
        const base = buffer.readUIntBE(p + 6, 4) * 2 + (buffer[p + 10] >> 7);
        const ext = ((buffer[p + 10] & 1) << 8) | buffer[p + 11];
        const pcr = base * 300 + ext; // ticks de 27 MHz
        if (firstPcr === null) { firstPcr = pcr; firstPcrOffset = p; }
        if (pcr >= firstPcr) { lastPcr = pcr; lastPcrOffset = p; }
      }
      q += 1 + length;
    }
    if (!(adaptation & 1) || !pusi || q >= p + 188) continue;
    q += 1 + buffer[q]; // pointer_field
    if (q + 3 > p + 188) continue;
    if (pid === 0 && buffer[q] === 0x00) {
      const length = ((buffer[q + 1] & 0x0f) << 8) | buffer[q + 2];
      for (let i = q + 8; i + 4 <= Math.min(p + 188, q + 3 + length - 4); i += 4) {
        const program = (buffer[i] << 8) | buffer[i + 1];
        const pmt = ((buffer[i + 2] & 0x1f) << 8) | buffer[i + 3];
        if (program !== 0) pmtPids.add(pmt);
      }
    } else if (pmtPids.has(pid) && buffer[q] === 0x02) {
      const length = ((buffer[q + 1] & 0x0f) << 8) | buffer[q + 2];
      const infoLength = ((buffer[q + 10] & 0x0f) << 8) | buffer[q + 11];
      let i = q + 12 + infoLength;
      const end = Math.min(p + 188, q + 3 + length - 4);
      while (i + 5 <= end) {
        const type = buffer[i];
        const esPid = ((buffer[i + 1] & 0x1f) << 8) | buffer[i + 2];
        const esLength = ((buffer[i + 3] & 0x0f) << 8) | buffer[i + 4];
  ...
  for (const type of streams.values()) {
    if (TS_VIDEO_TYPES[type] && !out.videoCodec) out.videoCodec = TS_VIDEO_TYPES[type];
    if (TS_AUDIO_TYPES[type] && !out.audioCodecs.includes(TS_AUDIO_TYPES[type])) out.audioCodecs.push(TS_AUDIO_TYPES[type]);
  }
  if (firstPcr !== null && lastPcr !== null && lastPcr > firstPcr && lastPcrOffset > firstPcrOffset) {
...
/* Muestra con ventana sostenida. La primera racha de bytes sale de la cache
   del motor y no dice nada del enjambre; se sigue leyendo sustainMs desde el
   primer byte y se mide la SEGUNDA mitad, que ya es caudal real. Ademas se
   guarda lo leido (hasta 8 MB) para sacar codec y bitrate del propio TS. */
function sampleScannerStream(pathname, timeoutMs = SCANNER_PROBE_TIMEOUT_MS, minBytes = SCANNER_SAMPLE_BYTES, sustainMs = SCANNER_SUSTAIN_MS) {
  return new Promise((resolve) => {
    const startedAt = Date.now();
    const keepBytes = 8 * 1024 * 1024;
    let request;
    let response;
    let bytes = 0;
    let done = false;
    ...
    const finish = (reason) => {
      ...
      const analysis = analyzeTransportStream(chunks.length ? Buffer.concat(chunks, kept) : Buffer.alloc(0));
      resolve({
        bytes, statusCode, contentType, reason,
    ...
    const open = (target, redirects = 0) => {
      request = http.get({
        ...
        incoming.on("data", (chunk) => {
          bytes += chunk.length;
          if (kept < keepBytes) {
            const room = Math.min(chunk.length, keepBytes - kept);
          ...
          if (!firstByteAt) {
            ...
            sustainTimer = setTimeout(() => finish("enough_data"), sustainMs);
          }
          if (Date.now() >= lateStartAt) lateBytes += chunk.length;
        });
...
function classifyScannerEvidence(evidence, minBytes = SCANNER_SAMPLE_BYTES) {
  ...
  /* Bytes y video no bastan: MOVISTAR PLUS de ELCANO recibia 10 MB de golpe
     de un par y despues 1,4 Mbit/s para un canal de 2,2. Si la entrada del
     enjambre en la segunda mitad de la ventana no cubre el bitrate del canal,
     la señal se para al rato y no puede ser verde. Sin bitrate conocido,
     1 Mbit/s es el minimo para que algo se vea. */
  const intakeKbps = Number.isFinite(Number(evidence?.intakeKbps)) && evidence?.intakeKbps !== null ? Number(evidence.intakeKbps) : null;
  const streamKbps = Number(evidence?.streamKbps) || 0;
  if (intakeKbps !== null && bytes >= minBytes) {
    const needed = streamKbps > 0 ? streamKbps * SCANNER_STARVED_RATIO : 1000;
...
async function probeAceCandidate(candidate, options = {}) {
  ...
  try {
    ...
    // 12 s, como el reproductor: con 8 un canal lento en encontrar pares salia
    // "sin señal" aqui y luego arrancaba bien en la pantalla
    const metaResult = await request(metaPath, Math.max(1000, Math.min(12000, deadline - Date.now())));
    ...
    const sampleBudget = Math.max(1000, Math.min(SCANNER_SUSTAIN_MS + 1500, remaining - 2500));
    /* Contador del motor a mitad de ventana: con el de despues da la entrada
       real del enjambre una vez pasada la rafaga inicial. */
    const midAfter = Math.floor(sampleBudget / 2);
    const midPromise = !statPath ? Promise.resolve(null) : new Promise((resolve) => setTimeout(resolve, midAfter))
      .then(() => request(statPath, 1500))
      .then((mid) => (mid.statusCode >= 200 && mid.statusCode < 300 ? { ...parseScannerStats(mid.body), at: Date.now() } : null))
      .catch(() => null);
    const sampled = await sample(playbackPath, sampleBudget, minBytes);
    const midStats = await Promise.race([midPromise, new Promise((resolve) => setTimeout(() => resolve(null), 1600))]);
    ...
    if (sampled.statusCode >= 200 && sampled.statusCode < 300 && sampled.bytes >= 16 * 1024) {
      if (sampled.videoCodec) {
        // la PMT ya dice el codec: se ahorra ffprobe y sus segundos
        media = {
          mediaValid: true,
    ...
    if (statPath) {
      try {
        ...
        if (after.statusCode >= 200 && after.statusCode < 300) { finalStats = parseScannerStats(after.body); finalAt = Date.now(); }
      } catch {}
    }
    ...
    const intakeKbps = intakeWindowMs >= 2000 && finalStats.downloaded >= midStats.downloaded
      ? Math.round(((finalStats.downloaded - midStats.downloaded) * 8) / intakeWindowMs)
      : null;
    const classification = classifyScannerEvidence({ ...sampled, ...finalStats, downloadedDelta, intakeKbps, ...media }, minBytes);
    return {
      ...classification,
      ...
      downloadedDelta,
      rateKbps: Number.isFinite(Number(sampled.rateKbps)) && sampled.rateKbps !== null ? Number(sampled.rateKbps) : null,
      intakeKbps,
...
function playerVerdictHeld(id, now = Date.now()) {
  const cached = scannerCache.get(id);
  return Boolean(cached && cached.by === "player" && now - cached.cachedAt < PLAYER_VERDICT_HOLD_MS);
}

/* Unico sitio donde se escribe un veredicto, venga del comprobador o del
   reproductor. Antes cada prueba pisaba a la anterior sin mas, y la misma
   fuente pasaba de "verificada" a "sin señal" y vuelta segun saliera la
   muestra de siete segundos:
   - lo que cuenta el reproductor no lo pisa el comprobador durante
     PLAYER_VERDICT_HOLD_MS;
   - una fuente verificada que falla UNA prueba queda "floja" (intermitente);
     solo un segundo fallo seguido la da por muerta.
   Lo que cuenta el reproductor se copia ademas a los trabajos abiertos, para
   que las demas pantallas del mismo partido lo vean al momento. */
function recordScannerVerdict(id, result, now = Date.now()) {
  const previous = scannerCache.get(id);
  const byPlayer = result?.by === "player";
  if (!byPlayer && playerVerdictHeld(id, now)) return { ...previous, cached: true };
  let verdict = { ...result, by: byPlayer ? "player" : "scanner" };
  if (!byPlayer && verdict.state === "failed" && !SCANNER_HARD_FAILURES.has(verdict.reason)
      && previous?.state === "working" && now - previous.cachedAt <= SCANNER_GOOD_TTL_MS) {
    verdict = { ...verdict, state: "weak", reason: "intermittent" };
  }
  scannerCache.set(id, { ...verdict, cachedAt: now });
  if (byPlayer) {
    for (const job of scannerJobs.values()) {
      ...
      for (const candidate of job.candidates) {
        ...
        Object.assign(candidate, { state: verdict.state, reason: verdict.reason, checkedAt: verdict.checkedAt, cached: true });
...
function pruneScoresCache(now = Date.now()) {
  for (const [clave, entry] of scoresCache) {
    if (entry.pending) continue;
    if (now - (Number(entry.expiresAt) || 0) > SCORES_CACHE_STALE_MS) scoresCache.delete(clave);
  }
}
...
function scannerJobPayload(job) {
  const candidates = job.candidates.map((item) => ({
    ...
    rateKbps: Number.isFinite(Number(item.rateKbps)) && item.rateKbps !== null ? Number(item.rateKbps) : null,
    intakeKbps: Number.isFinite(Number(item.intakeKbps)) && item.intakeKbps !== null ? Number(item.intakeKbps) : null,
    streamKbps: item.streamKbps || 0,
    reason: item.reason || "",
...
function enqueueScannerJob(job, priority = job?.priority === true) {
  ...
  setImmediate(drenarColaDelComprobador);
}
...
/* El drenador se lanza desde setImmediate sin nadie que espere la promesa.
   Si dentro reventaba algo -disco lleno al anotar un informe, por ejemplo-
   la promesa quedaba rechazada sin capturar y Node 24 tumba el proceso
   entero: se perdian precargas, cache del comprobador y las sesiones de
   remux. Ahora se registra y se sigue. */
function drenarColaDelComprobador() {
  drainScannerQueue().catch((error) => console.error(`[scanner] fallo drenando la cola: ${error?.stack || error}`));
}
...
async function drainScannerQueue() {
  ...
  try {
    while (scannerQueue.length) {
      ...
      /* La fuente que alguien esta viendo no se vuelve a probar aunque se
         fuerce (precarga en directo, Rebuscar): el comprobador le quitaba
         pares al motor principal en el mismo enjambre, y el reproductor ya
         cuenta como va. */
      const cached = candidate.force === true && !playerVerdictHeld(candidate.id) ? null : scannerCacheHit(candidate.id);
      if (cached) {
        ...
        const verdict = recordScannerVerdict(candidate.id, result);
        Object.assign(candidate, verdict, { cached: verdict.cached === true });
        const retry = scannerRetryPlan(verdict, candidate.attempts);
        if (retry) {
    ...
    scannerBusy = false;
    if (scannerQueue.length) setImmediate(drenarColaDelComprobador);
  }
}
...
/* "M+ LALIGA HDR" y "M+ LALIGA" son el mismo canal: cuentan como uno y se
   queda el rotulo mas corto, sea cual sea el orden en que lleguen. Asi el
   tope de rotulos no se gasta en variantes de calidad y no deja fuera al
   canal distinto que venia detras. */
function claveDeCanalSinVariante(channel) {
  return normalizeChannelKey(channel).split(" ")
    .filter((token) => token && !CHANNEL_VARIANT_TOKENS.has(token)).join(" ");
}
...
function resolutionChannels(values) {
  ...
  for (const raw of Array.isArray(values) ? values : [values]) {
    ...
    if (posicion.has(variante)) {
      const index = posicion.get(variante);
      if (key.split(" ").length < normalizeChannelKey(output[index]).split(" ").length) output[index] = channel;
      continue;
    }
...
/* Lo que cuenta el reproductor, traducido a veredicto del comprobador. */
function veredictoDelReproductor(resultado, segundos) {
  if (resultado === "arranco" || resultado === "sigue") return { state: "working", reason: "player_ok" };
  if (resultado === "cayo" && segundos >= STATS_SHORT_PLAY_S) return { state: "weak", reason: "player_dropped" };
...
function registrarResultadoDeFuente(state, datos) {
  ...
  if (!id || !["arranco", "fallo", "cayo", "sigue"].includes(resultado)) {
  ...
  recordScannerVerdict(id, {
    ...veredictoDelReproductor(resultado, segundos),
    by: "player",
    checkedAt: new Date(ahora).toISOString(),
  }, ahora);
  // "sigue" solo renueva el veredicto mientras se ve: no es un intento nuevo
...
/* La agenda lista varios canales por partido y NO son equivalentes. Para el
   Barcelona-Athletic anuncia ["DAZN LaLiga", "DAZN", "LaLiga TV Bar"]: "DAZN"
   es la MARCA y "DAZN LaLiga" es el canal donde dan ese partido. Tratarlos
   igual hacia que un vinculo guardado sobre la marca -"DAZN" apuntando a
   "DAZN 1"- se pusiera el primero y abriera otro canal, con otro deporte.

   Un canal pedido es generico cuando sus palabras son un subconjunto ESTRICTO
   de las de otro canal pedido: "dazn" dentro de "dazn laliga". No basta con
   contar palabras, porque "LaLiga TV Bar" y "DAZN" no compiten entre si. */
/* HDR, Bar o UHD son el MISMO canal en otra calidad o para otro local, no
   un canal mas concreto. Contarlas hacia que "M+ LALIGA" pareciera la marca
   de "M+ LALIGA HDR" y anulaba la regla justo cuando mas falta hacia. Ojo:
   siguen contando como palabra propia del canal -"LaLiga TV Bar" no cabe
   dentro de "DAZN LaLiga"-; solo dejan de hacer mas concreto a otro. */
const CHANNEL_VARIANT_TOKENS = new Set(["hdr", "bar", "uhd", "4k", "fhd", "hd"]);

function palabrasDeCanal(valor) {
  return new Set(normalizeChannelKey(valor).split(" ")
    .filter((token) => token && !CHANNEL_FILLER_TOKENS.has(token)));
}

function canalEsGenerico(canal, pedidos) {
  ...
  return (pedidos || []).some((otro) => {
    ...
    /* Si lo unico que sobra son coletillas de calidad o de local, no es un
       canal mas concreto: es el mismo canal en otra version. */
...
function mergeResolutionCandidates(candidates, options = {}) {
  ...
  /* Lo generico va DESPUES de lo concreto, y por encima del resto de reglas:
     esto es identidad de canal, no preferencia. Solo se aparta si de verdad
     hay algo casado con el canal concreto; si el partido solo se anuncia por
     la marca, la marca es lo unico que hay y no se toca nada. */
  const pedidos = Array.isArray(options.requestedChannels) ? options.requestedChannels : [];
  /* Segunda pista, esta con datos y no con palabras: si de otro canal pedido
     hay señales EXACTAS en la biblioteca y de este solo hay familia o un
     vinculo, este es la marca. Asi "DAZN" se aparta detras de "M+ LALIGA" en
     un partido anunciado en los dos, aunque la agenda no anuncie ningun
     "DAZN algo" mas concreto. Si ningun canal pedido tiene señal exacta, no
     hay con que comparar y no se aparta nada. */
  const conExacto = new Set(vivas
    .filter((c) => c.source !== "saved" && !c.soloFamilia && c.score >= RESOLUTION_EXACT_SCORE)
    .map((c) => normalizeChannelKey(c.matchedChannel)));
  const sinExacto = (candidate) => conExacto.size > 0
    && (candidate.soloFamilia || candidate.source === "saved")
    && !conExacto.has(normalizeChannelKey(candidate.matchedChannel));
  const esGenerica = (candidate) => canalEsGenerico(candidate.matchedChannel, pedidos) || sinExacto(candidate);
  ...
  for (const nivel of porNivel) for (const pertenece of GRUPOS) {
...
async function resolveFootballChannel(state, values, search = searchAceStreams, options = {}) {
  ...
  /* Lo aprendido -vinculos confirmados, fuentes en cuarentena- se aplica una
     sola vez, sobre todas las capas juntas y antes del umbral: la subida a 98
     de una fuente confirmada tiene que contar para que pase el corte. Antes
     se aplicaba dos veces, la segunda sobre lo ya aplicado. */
  const learnedCandidates = applyLearnedSourceRules(state, channels, [...bindings, ...semanticResult.candidates]);
  ...
  const candidates = mergeResolutionCandidates(
    qualified,
    research
      ? { sourceOrder: ["favorites", "m3u", "acestream"], sourceStats: state.sourceStats, requestedChannels: channels }
      : { sourceStats: state.sourceStats, requestedChannels: channels },
  );
  const ai = {
  ...
  /* Y "manda sobre todo" tiene un limite: si el orden por identidad de canal
     lo ha apartado por ser la marca, ya no decide. Buscarlo en cualquier
     posicion lo devolvia a la cabeza y abria DAZN 1 en un partido de M+ LALIGA. */
...
function restartAceStream() {
  return new Promise((resolve, reject) => {
    ...
    const controlReq = http.request({
      ...
      method: "POST",
      timeout: 8000,
      headers: ENGINE_CONTROL_TOKEN ? { "x-engine-token": ENGINE_CONTROL_TOKEN } : {},
    }, (controlRes) => {
      controlRes.resume();
      controlRes.on("end", () => {
...
async function handleRequest(req, res) {
  try {
    ...
    /* Siempre el cuerpo ANTES que el estado. Con "fn(readState(), await
       readBody(req))" el estado se leia al llegar las cabeceras y el cuerpo
       podia tardar; cualquier escritura entre medias -otro dispositivo, el
       comprobador, el auto-sync- quedaba pisada por esta copia vieja. */
    if (req.url === "/api/preferences") {
      if (req.method !== "POST") return send(res, 405, { error: "method_not_allowed" });
      const body = await readBody(req);
      return send(res, 200, updateFootballPreferences(readState(), body));
    }

    if (req.url === "/api/football/bind") {
      if (req.method !== "POST") return send(res, 405, { error: "method_not_allowed" });
      const body = await readBody(req);
      return send(res, 200, saveChannelBinding(readState(), body));
    }

    if (req.url === "/api/sources/report") {
      if (req.method !== "POST") return send(res, 405, { error: "method_not_allowed" });
      const body = await readBody(req);
      return send(res, 200, reportSource(readState(), body));
    }
    ...
    if (req.url === "/api/sources/feedback") {
      if (req.method !== "POST") return send(res, 405, { error: "method_not_allowed" });
      const body = await readBody(req);
      return send(res, 200, saveSourceFeedback(readState(), body));
    }
    ...
    /* Lo que el reproductor consulta cada pocos segundos para saber si otro
       dispositivo ha tomado el mando. Antes bajaba /api/state entero -unos
       100 KB con el directorio- para leer un solo campo. */
    ...
    if (req.url === "/api/remux/stop") {
      ...
      const token = String(body.token || "").trim().replace(/[^a-f0-9]/g, "").slice(0, 32);
      /* Parada de un enganche anterior (un reintento cuyo stop llega despues
         del /api/remux nuevo): no se toca la sesion que se esta viendo. */
      if (session && deviceId && token && session.clients.has(deviceId) && session.clients.get(deviceId) !== token) {
        return send(res, 200, { success: true, stopped: false, detached: false, stale: true });
      }
    ...
    if (req.url === "/api/remux" || req.url.startsWith("/api/remux?")) {
      ...
      const { session: s, token } = ensureRemux(ihParam ? "infohash" : "id", id, deviceId);
      ...
      for (;;) {
        ...
        const ready = remuxPlaylistStats(manifest);
        const waited = Date.now() - t0;
        if (ready && ready.segments >= 2 && ready.seconds >= REMUX_READY_SECONDS) break;
        if (ready && ready.segments >= 1 && waited > REMUX_READY_WAIT_MS) break;
        if (waited > REMUX_START_TIMEOUT_MS) return send(res, 504, { error: "remux_timeout" });
        await new Promise((r) => setTimeout(r, 400));
      }
      return send(res, 200, { url: `/remux/${id.toLowerCase()}/index.m3u8`, token });
    }
    ...
    if (req.url === "/api/streams/sync") {
      ...
      let streams;
      try {
        const text = await fetchDirectoryText(url);
        streams = (type === "m3u" ? parseM3u(text) : parseHtml(text)).slice(0, MAX_WEB_STREAMS);
        if (!streams.length) throw new Error("empty_directory");
      } catch (error) {
        // un directorio ya guardado conserva su cache y apunta el motivo
        if (snapshotSource) anotarFalloDeDirectorio(snapshotSource.id, error);
        throw error;
      }
    ...
    const message = String(error?.message || "");
    if (safeErrors.has(message) || /^http_\d{3}$/.test(message)) {
      return send(res, status, { error: message });
    }
    /* Todo lo demas es un fallo del servidor, no del cliente: antes salia
       como 400 "bad_request" y sin rastro en el log, asi que un disco lleno
       o un TypeError parecian una peticion mal hecha. */
    console.error(`[api] ${req.method} ${req.url}:`, error?.stack || error);
    send(res, 500, { error: "internal_error" });
  }
}

/* Apunta en un directorio guardado cuando y por que fallo su ultima
   actualizacion, sin tocar su cache de canales. */
function anotarFalloDeDirectorio(sourceId, error) {
  const latest = readState();
  if (!latest.webSources.some((source) => source.id === sourceId)) return;
  const webSources = latest.webSources.map((source) => source.id === sourceId
    ? { ...source, lastErrorAt: new Date().toISOString(), lastError: motivoDeFallo(error) }
    : source);
...
async function autoSyncWeb() {
  ...
  for (const source of snapshot.webSources) {
    try {
      const text = await fetchDirectoryText(source.url);
...
function startServer() {
  ...
  const syncTimer = AUTO_SYNC_ENABLED ? setInterval(runAutoSync, WEB_SYNC_INTERVAL_MS) : null;
  ...
  if (AUTO_SYNC_ENABLED) runAutoSync();

  /* Una promesa rechazada sin capturar no debe tumbar el servidor entero. */
  process.on("unhandledRejection", (reason) => {
    console.error("[proceso] promesa rechazada sin capturar:", reason?.stack || reason);
  });
  /* Docker manda SIGTERM al parar o actualizar: se cierran las conexiones y
     se matan los ffmpeg del remux en vez de dejarlos huerfanos hasta que el
     contenedor muera por la fuerza. */
  const apagar = (signal) => {
    console.log(`[proceso] ${signal}: cerrando`);
