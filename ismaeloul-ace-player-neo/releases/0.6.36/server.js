...
/* Cuantas señales se ofrecen. Los hashes de AceStream caducan solos, asi que
   mas alternativas es mas probabilidad de que alguna tire. */
const RESOLUTION_MAX_CANDIDATES = 12;
...
function proveedorDeSeñal(candidate) {
  const coletilla = /-->\s*(.+)$/.exec(String(candidate.title || ""));
...
function mergeResolutionCandidates(candidates) {
  ...
  const hayExacto = todos.some((candidate) => !candidate.soloFamilia && candidate.score >= 92);
  const vivas = todos
    .filter((candidate) => !(hayExacto && candidate.soloFamilia))
    // el motor dice que no queda nadie compartiendo: no puede funcionar
    .filter((candidate) => candidate.availability !== 0);
  ...
  /* Primero lo tuyo, despues lo del buscador.
     ...
     2. tus listas M3U importadas, favoritos e historial
     3. el buscador del motor, como red de seguridad
     Dentro de cada grupo se reparte entre proveedores, para que un proveedor
     caido no copo la cabecera. */
  const GRUPOS = [
    (c) => c.source === "saved",
    (c) => c.source === "m3u" || c.source === "favorites" || c.source === "history",
    () => true,
  ];
  const salida = [];
  ...
  /* Si tus listas llenan las doce plazas, el buscador se queda fuera entero y
     te quedas sin red por si tus hashes han caducado todos a la vez. Se le
     guardan las dos ultimas, pero SOLO si hay algo que poner ahi: si no, esas
     plazas vuelven a tus listas. */
  const RESERVA_BUSCADOR = 2;
  const mias = salida.filter((c) => c.source !== "acestream");
  const delBuscador = salida.filter((c) => c.source === "acestream");
  if (!delBuscador.length) return mias.slice(0, RESOLUTION_MAX_CANDIDATES);
  const sitioParaMias = Math.max(0, RESOLUTION_MAX_CANDIDATES - Math.min(RESERVA_BUSCADOR, delBuscador.length));
  return [
    ...mias.slice(0, sitioParaMias),
    ...delBuscador.slice(0, RESOLUTION_MAX_CANDIDATES - Math.min(mias.length, sitioParaMias)),
  ].slice(0, RESOLUTION_MAX_CANDIDATES);
}
