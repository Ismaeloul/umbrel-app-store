...
async function fetchText(url, redirects = 0, visited = new Set(), deadline = Date.now() + FETCH_TOTAL_TIMEOUT_MS, maxBytes = MAX_BODY) {
  ...
  return new Promise((resolve, reject) => {
    ...
    const req = client.get(parsed, {
      ...
      headers: {
        "User-Agent": "AcePlayerNeo/0.6.57",
...
/* Descarga de un directorio: como fetchText -mismo filtro de IPs privadas-
   pero si una pasarela IPFS publica se satura (429) o cae (5xx) se prueba
   una vez la otra con la misma ruta. Si tampoco, se conserva el error
   original, que es el que ve el usuario. */
