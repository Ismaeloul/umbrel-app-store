...
const FOOTBALL_DAYS = Math.min(7, Math.max(3, Number.parseInt(process.env.FOOTBALL_DAYS, 10) || 5));
...
function channelMatchScore(left, right) {
  ...
  if (a.includes(b) || b.includes(a)) {
    ...
    return shortest >= 2 ? 86 - Math.min(14, Math.abs(a.length - b.length)) : 58;
  }
  ...
  return shared >= 2 && ratio >= 0.6 ? Math.round(58 + ratio * 24) : 0;
}
...
async function fetchText(url, redirects = 0, visited = new Set(), deadline = Date.now() + FETCH_TOTAL_TIMEOUT_MS) {
  ...
  return new Promise((resolve, reject) => {
    ...
    const req = client.get(parsed, {
      ...
      if (response.statusCode >= 300 && response.statusCode < 400 && response.headers.location) {
        ...
        clearTimeout(totalTimer);
        fetchText(nextUrl, redirects + 1, visited, deadline)
...
function normalizeFootballTime(value) {
  const match = String(value || "").match(/^(\d{1,2}):(\d{2})/);
  if (!match) return "Por confirmar";
  const hour = Number(match[1]);
  if (!Number.isInteger(hour) || hour < 0 || hour > 23) return "Por confirmar";
...
function normalizeFootballRows(rows) {
  ...
  for (const row of Array.isArray(rows) ? rows : []) {
    ...
    const time = normalizeFootballTime(row?.strTime);
    ...
    if (!match) {
      match = {
        ...
        home: event.home,
        away: event.away,
        competition: cleanTitle(row?.strLeague || row?.strCompetition, "Fútbol"),
...
async function fetchFootballSchedule() {
  ...
  const dates = Array.from({ length: FOOTBALL_DAYS }, (_, index) => addIsoDays(startDate, index));
  ...
  const matches = normalizeFootballRows(rows);
  return {
    success: true,
    generatedAt: new Date().toISOString(),
    timezone: FOOTBALL_TIMEZONE,
...
async function getFootballSchedule() {
  ...
  footballCache.pending = fetchFootballSchedule()
    .then((payload) => {
...
function scoreResolutionCandidate(channels, item, source) {
  ...
  for (const channel of channels) {
    const candidateScore = channelMatchScore(channel, item.title);
...
function libraryResolutionCandidates(state, channels) {
  ...
  for (const [source, items] of [
    ...
    for (const item of items) {
      ...
      if (candidate.score >= 58) output.push(candidate);
    }
  }
...
async function resolveFootballChannel(state, values, search = searchAceStreams) {
  ...
  if (bindings[0]) {
    return { success: true, status: "found", channels, checked, candidate: bindings[0], candidates: bindings.slice(0, 1), engineAvailable: true };
  }

  checked.push("m3u", "library");
  const local = libraryResolutionCandidates(state, channels);
  if (local[0]?.score >= 92) {
    return { success: true, status: "found", channels, checked, candidate: local[0], candidates: local.slice(0, 4), engineAvailable: true };
  }

  checked.push("acestream");
  ...
  const candidates = mergeResolutionCandidates([...local, ...remote]);
  if (candidates[0]?.score >= 96 && (!candidates[1] || candidates[1].score < 96)) {
    return { success: true, status: "found", channels, checked, candidate: candidates[0], candidates, engineAvailable };
  }
