  ...
  try {
    while (scannerQueue.length) {
      ...
      if (cached) {
        ...
        const nonRetryable = result.reason === "unsupported_codec";
        if (result.state === "failed" && candidate.attempts < 2 && !nonRetryable) {
...
async function resolveFootballChannel(state, values, search = searchAceStreams, options = {}) {
  ...
  const candidates = applyLearnedSourceRules(
    state, channels, mergeResolutionCandidates([...bindings, ...qualified]),
  );
  const ai = {
...
async function handleRequest(req, res) {
  try {
    ...
    if (req.url === "/api/football/resolve" || req.url.startsWith("/api/football/resolve?")) {
      ...
      const preheated = reusablePreheat(url.searchParams.get("match"));
      ...
      if (preheated) {
        ...
        result = await resolveFootballChannel(currentState, announcedChannels, searchAceStreams, { program });
      }
